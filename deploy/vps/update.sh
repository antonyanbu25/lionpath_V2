...
if [[ "${UPDATE_POST_RESET:-}" != "1" ]]; then
  echo "=== Fetching origin/2.0.8.2 ==="
  bash "$DEPLOY_DIR/git-fetch-origin.sh" "$REPO_ROOT" "2.0.8.2"

  echo "=== Resetting to origin/2.0.8.2 (keeps .env — gitignored) ==="
  cd "$REPO_ROOT"
  git checkout 2.0.8.2 2>/dev/null || git checkout -B 2.0.8.2
...
if ! grep -q 'precall.css?v=2.0.8.2' "$REPO_ROOT/web/index.html" 2>/dev/null; then
  echo "ERROR: web/index.html missing 2.0.8.2 precall cache-bust after reset — git pull did not apply." >&2
  echo "       Run: bash $DEPLOY_DIR/git-auth-diagnose.sh" >&2
  exit 1
fi

if ! grep -q 'postcall-intake-card' "$REPO_ROOT/web/index.html" 2>/dev/null; then
  echo "ERROR: web/index.html missing postcall-intake-card — git reset did not apply 2.0.8.2 postcall UI." >&2
  exit 1
fi
