...
-- Child rows
\echo '== id_registry backfill: account =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'account', a.public_id, a.id
FROM account a
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill: deal =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'deal', d.public_id, d.id
FROM deal d
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill: contact =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'contact', c.public_id, c.id
FROM contact c
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill: activity =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'activity', a.public_id, a.id
FROM activity a
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill: pre_call =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'pre_call', p.public_id, p.id
FROM pre_call p
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill: post_call =='
INSERT INTO id_registry (entity_type, public_id, internal_id)
SELECT 'post_call', p.public_id, p.id
FROM post_call p
ON CONFLICT DO NOTHING;

\echo '== id_registry backfill complete =='
