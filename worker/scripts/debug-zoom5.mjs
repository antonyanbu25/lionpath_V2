...
for (const [k, v] of Object.entries(pi)) {
  if (/transcript|caption|cc|vtt|speech|subtitle|asr|audio/i.test(k)) {
    console.log(k, ":", typeof v === "string" ? v.slice(0, 150) : JSON.stringify(v)?.slice(0, 200));
  }
}
console.log("disableDownload:", pi.disableDownload, "showCcDropDown:", pi.showCcDropDown);
