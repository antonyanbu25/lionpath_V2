...
assert.equal(compact.meta.linkedinProspectProfiles?.[0]?.profile?.name, "Pat Lee", "keep compact linkedin profile");

globalThis.localStorage = makeQuotaStorage(3200);
localStorage.setItem(
  BRIEFS_KEY,
  JSON.stringify([
    compactBriefForStorage(sampleBrief("old", 700)),
    compactBriefForStorage(sampleBrief("older", 700)),
  ]),
);
...
saveBriefToSidebar(
  ...
  minimalV8Prep({ blob: "n".repeat(700) }),
...
assert.ok(Buffer.byteLength(JSON.stringify(stored), "utf8") <= 3200, "stored payload fits quota");
