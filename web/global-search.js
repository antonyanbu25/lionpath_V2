...
import { readFieldValueAsync, fillShadowFieldHeight } from "./crayons-ui.js";
import {
  buildSearchIndex,
  searchIndex,
  recentFromIndex,
...
const TYPE_ORDER = ["account", "contact", "brief", "call"];
...
export function initGlobalSearch(deps) {
  ...
  fillShadowFieldHeight(topbarInput);
  ...
  function selectResult(item) {
    ...
    if (item.type === "account") {
      ...
      // Contacts live on an account — open the parent account, focused on the contact.
  ...
  function renderResults(results, query) {
    ...
    if (!results.length) {
      resultsEl.innerHTML = query
        ...
        : `<p class="global-search-empty muted">Type to search accounts, contacts, briefs, and calls</p>`;
      return;
    }
