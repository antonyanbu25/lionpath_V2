...
```typescript
{
  ...
  metadata?: {
    meddpicc?: MeddpiccRollup;
    /** Account Executive on the deal (free-form; AE is not a system user). */
...
Auto-created deals are titled `"<Account> - Deal <N> - <yyyy-mm-dd>"` (N increments per account, ordered by creation). Legacy `"New business"`/`"Expansion"` titles are lazily upgraded on read (`ensureDealTitle`) and by `worker/scripts/migrate-deal-titles.mjs`. AE is captured at the post-call confirm gate and stored on `metadata.ae`.
...
PostCall additionally carries `identities` — the call's participants: `{ aeName, aeEmail, primarySeUserId, secondarySeUserIds[], contactIds[] }`. Primary/secondary SE mirror the account `seTeam` at analysis time; when a second SE analyzes a call on an existing account they are auto-added to the team as `secondary` (up to `MAX_SE_TEAM_SIZE`). Titled `"<Type> with <Account>"` (e.g. `"Discovery with Acme"`).
