...
Expect `portal-build" content="2.1.24"`, `postcall.css?v=2.1.24` (aligned with portal-build), `pc-account-deal-preview`, `postcall-intake-card`, and `workerBuild` including `2.1.5`. Portal should not show the "Speed fixes not deployed" banner when both portal and worker are on 2.1.x.
